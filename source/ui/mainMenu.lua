...
buttons = {}
buttons[1] = {361, 380, 390, 72, "Nouvelle Partie"}
...
function menuDraw()
  ...
  if gameState.room == "rmMainMenu" then
    ...
    love.graphics.printf("A PIECE OF EPSI", 0, 140 * scale, gameWidth * scale, "center")
    ...
    for _,b in ipairs(buttons) do
      ...
      if buttons:mouseCheck(b) then -- if the mouse is over the button...
        ...
        if bText == "Nouvelle Partie" then
          buttons.message = "Démarrer une nouvelle partie - Supprimera l'ancienne"
        elseif bText == "Continue" then
          buttons.message = "Continuez là où vous vous êtes arrêté"
        elseif bText == ".sound" then
          buttons.message = "Activer ou désactiver la musique et les effets sonores"
        elseif bText == ".github" then
          buttons.message = "Voir le code sur GitHub"
        end
  ...
  for i,b in ipairs(self) do
    ...
    if buttons:mouseCheck(b) then
      ...
      elseif i == 4 then -- GitHub button
        ...
        love.system.openURL("https://github.com/skyneur")
